...
it('returns 200 & successfully receives requested contact', async () => {
  ...
  expect(response.body.data._id).toEqual(contact._id);
});

it('should return error if objectid is not valid objectid', async () => {
  const response = await request(app)
    .get(`/api/contacts/12345678900`)
    .set('Cookie', cookie)
    .expect(400);
...
it('should return error if objectid is not existed', async () => {
  ...
  expect(response.body.message).toEqual('Can not find contact with provided id');
});
