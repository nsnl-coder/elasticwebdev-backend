...
it('shoud update the contact', async () => {
...
it('should return error if objectid is not valid', async () => {
  const { body } = await request(app)
    .put('/api/contacts/not-valid-objectid')
    .send({
      test_number: 24,
    })
    .set('Cookie', cookie)
...
it('should return error if objectid does not exist in db', async () => {
  const { body } = await request(app)
    .put('/api/contacts/00000020f51bb4362eee2a4d')
    .send({
      test_number: 24,
    })
    .set('Cookie', cookie)
  ...
  expect(body.message).toEqual('Can not find contact with provided id');
});
